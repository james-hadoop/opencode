...
class FeishuChannelService:
    ...
    def __init__(self, config: FeishuConfig):
        ...
        self._received_messages: list[dict] = []  # 调试用：存储接收到的消息
    ...
    async def _connect_websocket(self):
        ...
        # 消息处理函数
        def handle_message(data: im.v1.P2ImMessageReceiveV1):
            logger.info(f"SDK handle_message called with data: {data}")
            try:
                event = data.event
                if not event:
                    logger.info("No event in message data")
                    return
                ...
                if msg_type == "text":
                    ...
                    if self.message_store.is_duplicate(context.message_id):
                        logger.info(f"Duplicate message ignored: {context.message_id}")
                        return
                    
                    # 使用线程安全的 queue 将消息传递到主线程
                    self._msg_queue.put((context,))
                ...
                logger.info(f"Received message: {msg_type} from {sender_id} in {chat_id}")
        ...
        logger.info("WebSocket client created, starting connection in background thread...")
        
        import threading
        ws_thread = threading.Thread(target=self._ws_client.start, daemon=True)
        ws_thread.start()
    ...
    async def _dispatch_message(self, context: FeishuMessageContext):
        """分发消息到注册的回调"""
        # 检查配对（仅 DM 消息）
        if hasattr(context, 'chat_type') and context.chat_type == ChatType.P2P:
            ...
            if sender_id and not self.pairing_store.is_approved(sender_id):
                ...
                # 发送配对消息
                pairing_message = self._build_pairing_message(sender_id, code)
                try:
                    await self.send_message(to=f"user:{sender_id}", text=pairing_message)
                except Exception as e:
